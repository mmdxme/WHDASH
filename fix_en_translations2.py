...
# Find the line where fa dict starts
fa_start = None
...
# The English dict should close at fa_start - 1
en_close = fa_start - 1
print(f"English dict closes at line {en_close + 1}: {repr(lines[en_close])}")

# The project translations should be inserted BEFORE the "    }," line
# So we insert at position en_close
...
# Insert project translations BEFORE the "    }," line that closes English dict
# The line at en_close is "    }," - we want to insert AFTER the last actual content line
# and BEFORE the closing brace

# Actually, the line at en_close is "    }," which is the close line
# We need to insert BEFORE this line, replacing it with content + close

# Get lines up to but NOT including the close line
new_lines = lines[:en_close]  # Everything up to but not including "    },"
# Add project translations
new_lines.append(project_translations_en)
# Add the close line
new_lines.append('    },\n')
# Add the rest (fa dict and beyond)
new_lines.extend(lines[fa_start:])
