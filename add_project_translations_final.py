...
# Properly add project translations to English dict in translations.py

with open('translations.py', 'r', encoding='utf-8') as f:
    lines = f.readlines()

# Find where fa dict starts
fa_start = None
...
# The line BEFORE fa dict should be "    }," which closes English dict
en_close = fa_start - 1
print(f"English dict closes at line {en_close + 1}: {repr(lines[en_close])}")

# Now insert project translations BEFORE the "    }," line
# The key insight: we need to INSERT content before line en_close, then keep the "    }," line
...
# Create new content
# Lines 0 to en_close-1 (everything up to but NOT including the "    }," line)
# Then project translations
# Then "    },\n" (the close line)
# Then lines fa_start onwards

new_content = ''.join(lines[:en_close]) + project_translations + '    },\n' + ''.join(lines[fa_start:])
