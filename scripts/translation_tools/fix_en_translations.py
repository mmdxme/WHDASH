...
# Fix translations.py - add project translations to English dict
# The English dict is at lines 8-8185, so we need to insert before line 8186 (where fa starts)

with open('translations.py', 'r', encoding='utf-8') as f:
    lines = f.readlines()

# The English dict ends at line 8185 (0-indexed: 8184)
# The fa dict starts at line 8186 (0-indexed: 8185)
# So we need to insert project translations at line 8185 (0-indexed: 8184), which is the last line of the English dict

# Actually, let me find the EXACT line where English dict closes
# Looking for line that is "    }," followed by the fa dict start

# Find the line before "    'fa': {"
insert_line = None
for i in range(len(lines)):
    if lines[i].strip() == "'fa': {":
        insert_line = i  # This is where fa dict starts
        break

print(f"fa dict starts at line {insert_line + 1}")
print(f"Line before fa dict: {repr(lines[insert_line - 1])}")

# The English dict closes at line insert_line - 1 (which should be "    },")
print(f"English dict closes at line {insert_line}")

# The project translations should be inserted BEFORE this line
# But actually - the English dict close is at insert_line - 1, so we insert at insert_line - 1

# Actually, let me just insert right before the fa dict start
# Insert at line insert_line - 1 (the "}," line)

project_translations_en = """
        # Project Management
...
# Insert at line insert_line - 1 (the closing line of English dict)
# Actually, insert right before the "    }," line that closes the English dict

insert_at = insert_line - 1  # This is the line with "    },"
print(f"Inserting project translations at line {insert_at + 1}")
print(f"Current content at insert line: {repr(lines[insert_at])}")

# Create new content
new_lines = lines[:insert_at] + [project_translations_en, '\n    },\n'] + lines[insert_at+1:]
...
try:
    ...
    print("Updated translations.py parses OK!")
except SyntaxError as e:
    print(f"Updated translations.py FAILS: {e.msg} at line {e.lineno}")