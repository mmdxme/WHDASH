...
# Re-add project management translations

# Find where the English dict ends and add project translations before the closing brace

with open('translations.py', 'r', encoding='utf-8') as f:
    content = f.read()

# Find the end of the English dict - look for "    },\n}" at the end
# Actually, find the last "    }" in the English section

# The English dict ends with:
#         'loading_wait': 'Please wait...',
#     }
# }

# Find where that is and add project translations before the closing

# Actually, simpler: find the last "    },\n" and add project translations before it

# Find the position of the final "    }\n}" structure
lines = content.split('\n')

# Find the line that closes the English dict
# It's the last "    }" before the final "\n}"
en_close_line = None
for i in range(len(lines)-1, -1, -1):
    if lines[i].strip() == '},':
        # Check if this is the English dict close (indent should be 4)
        indent = len(lines[i]) - len(lines[i].lstrip())
        if indent == 4:
            en_close_line = i
            break

print(f"English dict closes at line {en_close_line + 1}")

# Add project translations before line en_close_line
...
# Insert project translations before the English dict close
# Find the line with "    }," that closes the English dict
lines = content.split('\n')
insert_line = None
for i in range(len(lines)-1, -1, -1):
    if lines[i].strip() == '},':
        # Check if this is likely the English dict close (should be at 4-space indent)
        indent = len(lines[i]) - len(lines[i].lstrip())
        if indent == 4:
            insert_line = i
            break
...
if insert_line:
    print(f"Inserting project translations at line {insert_line + 1}")
    # Insert the translations before this line
    new_lines = lines[:insert_line] + [project_translations] + lines[insert_line:]
    new_content = '\n'.join(new_lines)
else:
    print("Could not find English dict close line!")
    new_content = content
