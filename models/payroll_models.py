...
def seed_payroll_default_data():
    ...
    try:
        ...
        default_components = [
            # Earnings
            ('BASIC', 'Basic Salary', 'Basic Salary', 'Basic Salary', 'Basic Salary', 'Basic Salary', 'Basic Salary', 'Basic Salary', 'Basic Salary', 'Earning', 'Salary', 'Base salary component', 1, 1, 1, 'Fixed', 0, 0, None, None, 1, 0, 0, 1),
            ('HRA', 'House Rent Allowance', 'بدل إيجار السكن', 'کمک هزینه مسکن', 'Housing Allowance', 'Allocation de logement', '房屋补贴', 'Mietzulage', 'Allowance', 'Housing', 'House rent allowance', 1, 1, 0, 'Percentage', 0, 25.0, 'BASIC', None, None, 2, 0, 0, 1),
            ('TRANSPORT', 'Transport Allowance', 'بدل انتقالات', 'کمک هزینه حمل و نقل', 'Transport Allowance', 'Allocation transport', '交通补贴', 'Transportzulage', 'Allowance', 'Transport', 'Transport allowance', 0, 0, 0, 'Fixed', 0, 0, None, None, 3, 0, 0, 1),
            ('MEDICAL', 'Medical Allowance', 'بدل طبي', 'کمک هزینه پزشکی', 'Medical Allowance', 'Allocation médicale', '医疗补贴', 'Medizinische Zulage', 'Allowance', 'Medical', 'Medical allowance', 0, 0, 0, 'Fixed', 0, 0, None, None, 4, 0, 0, 1),
            ('OT_REG', 'Overtime - Regular', 'عمل إضافي - عادي', 'اضافه کاری - عادی', 'Overtime - Regular', 'Heures supplémentaires - Régulier', '加班 - 常规', 'Überstunden - Regulär', 'Earning', 'Overtime', 'Regular overtime hours', 1, 1, 0, 'Hourly', 0, 0, None, None, 10, 0, 0, 1),
            ('OT_Wknd', 'Overtime - Weekend', 'عمل إضافي - نهاية الأسبوع', 'اضافه کاری - آخر هفته', 'Overtime - Weekend', 'Heures supplémentaires - Week-end', '加班 - 周末', 'Überstunden - Wochenende', 'Earning', 'Overtime', 'Weekend overtime', 1, 1, 0, 'Hourly', 0, 0, None, None, 11, 0, 0, 1),
            ('OT_Hol', 'Overtime - Holiday', 'عمل إضافي - يوم عطل', 'اضافه کاری - تعطیل', 'Overtime - Holiday', 'Heures supplémentaires - Jour férié', '加班 - 假日', 'Überstunden - Feiertag', 'Earning', 'Overtime', 'Holiday overtime', 1, 1, 0, 'Hourly', 0, 0, None, None, 12, 0, 0, 1),
            ('BONUS', 'Bonus', 'مكافأة', 'پاداش', 'Bonus', 'Prime', '奖金', 'Bonus', 'Earning', 'Bonus', 'Performance bonus', 1, 1, 0, 'Variable', 0, 0, None, None, 15, 0, 1, 1),
            ('COMMISSION', 'Commission', 'عمولة', 'کمیسیون', 'Commission', 'Commission', '佣金', 'Provision', 'Earning', 'Variable', 'Sales commission', 1, 1, 0, 'Variable', 0, 0, None, None, 16, 0, 1, 1),
            # Deductions
            ('TAX', 'Income Tax', 'ضريبة الدخل', 'مالیات', 'Income Tax', 'Impôt sur le revenu', '所得税', 'Einkommensteuer', 'Tax', 'Tax', 'Income tax deduction', 0, 0, 0, 'calculated', 0, 0, None, None, 50, 0, 1, 1),
            ...
            ('LOAN_RECV', 'Loan Recovery', 'استرداد قرض', 'بازپرداخت وام', 'Loan Recovery', 'Remboursement prêt', '贷款还款', 'Kreditrückzahlung', 'Deduction', 'Loan', 'Loan installment recovery', 0, 0, 0, 'calculated', 0, 0, None, None, 55, 0, 0, 1),
            ('ADV_RECV', 'Advance Recovery', 'استرداد سلفة', 'بازپرداخت پیش پرداخت', 'Advance Recovery', 'Remboursement avance', '预付款还款', 'Vorschussrückzahlung', 'Deduction', 'Advance', 'Salary advance recovery', 0, 0, 0, 'calculated', 0, 0, None, None, 56, 0, 0, 1),
            ('ABSENT', 'Absent Deduction', 'خصم الغياب', 'کسر غیبت', 'Absent Deduction', 'Déduction absence', '缺勤扣除', 'Abwesenheitsabzug', 'Deduction', 'Attendance', 'Deduction for absent days', 0, 0, 0, 'calculated', 0, 0, None, None, 60, 0, 0, 1),
            ('LATE', 'Late Deduction', 'خصم التأخير', 'کسر دیرکرد', 'Late Deduction', 'Déduction retard', '迟到扣除', 'Verspätungsabzug', 'Deduction', 'Attendance', 'Deduction for late arrivals', 0, 0, 0, 'calculated', 0, 0, None, None, 61, 0, 0, 1),
            ('PENALTY', 'Penalty Deduction', 'خصم جزائي', 'کسر جریمه', 'Penalty Deduction', 'Déduction pénalité', '罚款扣除', 'Strafabzug', 'Deduction', 'Penalty', 'Penalty deduction', 0, 0, 0, 'Variable', 0, 0, None, None, 65, 0, 1, 1),
            # Arrears/Adjustments
            ('ARREARS', 'Arrears', 'العدادات', 'معوقات', 'Arrears', 'Arriérés', '欠款', 'Rückstände', 'Arrears', 'Arrears', 'Arrears payment', 1, 1, 0, 'Variable', 0, 0, None, None, 70, 0, 0, 1),
            ('ADJUST', 'Adjustment', 'تعديل', 'تعدیل', 'Adjustment', 'Ajustement', '调整', 'Anpassung', 'Adjustment', 'Adjustment', 'Manual adjustment', 1, 1, 0, 'Variable', 0, 0, None, None, 71, 0, 1, 1),
        ]
