...
from application.service_registry import services
from database import get_db


# Define WMS Blueprint
    ...
    def decorator(f):
        ...
        def decorated_function(*args, **kwargs):
            ...
            if not user_has_permission(user_id, 'wms', resource, action):
                ...
                return redirect(url_for('wms.wms_dashboard'))
...
def wms_csrf_required(f):
    ...
    def decorated_function(*args, **kwargs):
        if request.method in ('POST', 'PUT', 'PATCH', 'DELETE') and not validate_wms_csrf():
            ...
            return redirect(request.referrer or url_for('wms.wms_dashboard'))
        return f(*args, **kwargs)
    return decorated_function
...
def register_wms_routes(app, get_db):
    ...
    def wms_dashboard():
        ...
        # Use Service Layer for data aggregation
        data = services.wms_dashboard.get_dashboard_data(user_id)
